...
export default function WorldZonesPage() {
  ...
  return (
    <>
      {showVillainIntro && <VillainIntro onComplete={() => setVillainIntroDone(true)} />}
        ...
        <div className="container mx-auto mt-8 max-w-2xl space-y-4 px-4">
          {world.zones.length === 0 && (
            <div className="rounded-2xl border border-white/10 bg-white/5 p-8 text-center">
              ...
              <p className="mt-2 text-sm text-white/40">This world's missions are being prepared.</p>
            </div>
          )}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-white/30">Zone {i + 1}</span>
                    {isCompleted && (
                      <div className="flex items-center gap-0.5">
                        {[1, 2, 3].map((s) => (
                          <Star
                            key={s}
                            className={`h-3 w-3 ${s <= stars ? "fill-amber-400 text-amber-400" : "text-white/20"}`}
                          />
                        ))}
                      </div>
                    )}
                  </div>
                  <h3 className={`mt-0.5 font-bold ${isLocked ? "text-white/30" : "text-white"}`}>{zone.name}</h3>
                  <p className={`text-sm ${isLocked ? "text-white/15" : "text-white/50"}`}>{zone.description}</p>
                </div>
          ...
          {world.zones.length > 0 && (
            ...
            >
              ...
              <div>
                <span className="text-[10px] font-bold uppercase tracking-wider text-white/30">Boss Battle</span>
